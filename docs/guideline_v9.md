...
| `CLAUDE.md` | 에이전트 행동 규칙. 노후화하지 않는 항목만 포함한다 | 본 문서 다음. 아키텍처·버전·API 사실은 기술하지 않고 본 문서를 참조한다 |
...
`CLAUDE.md`는 커밋 `3aaa1e9`에서 삭제되어 현재 저장소에 존재하지 않는다. 복원 시에는 위 관할에 맞추어 재작성한다. 삭제 직전 최종본(2026-05-24)은 알고리즘 위치·도보 경로 API·이미지 저장소에 대해 현행과 다른 서술을 담고 있었으므로 그대로 복원하지 않는다.
