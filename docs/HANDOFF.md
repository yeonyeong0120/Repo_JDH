...
- §1 활동 종료 · §2 활동 취소 · §4 공유 여부 · §7 가입 확인 · §8 그룹 탈퇴 ·
  §10 그룹 개설 제한 · §11 이름 중복 · §17 로그아웃 · §18 회원 탈퇴 ·
  §21 구매 확인 · §22 구매 완료
- 이 중 **§2 는 버튼 의미가 뒤집혀 있었다** — 파괴적 선택('나가기')이 취소 자리에
  가로로 놓여 있었다. 세로 배치 + 안전한 선택을 위·다크로 바로잡았다.
- **§18 은 A형 팝업에서 B형 시트로 전면 교체**(`withdraw_sheet.dart` 신설).
  손실 목록 4행 + 동의 체크가 생겼고, 수치는 실제 계정에서 읽는다.
...
- **E형**: §19 탈퇴 완료 화면 — 아직 없다. 탈퇴 후 바로 로그인 화면으로 간다.
