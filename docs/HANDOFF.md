...
- **B형 시트**: §3 가이드 · §12 초대 · §14 신고 · §15 정렬 · §16 숫자 피커를 맞췄다.
  §16 은 프로필의 시트만 해당한다 — 회원가입 쪽은 시트가 아니라 화면 안
  인라인 스테퍼로 바뀌어 있어 이 명세의 대상이 아니다.
...
- **C형**: §5 퀘스트 완료 · §6 뱃지 획득을 맞췄다. 둘 다 **위계가 뒤집혀 있었다** —
  명세는 '퀘스트 완료'·'새 뱃지 획득'이 제목(19)이고 퀘스트명·뱃지명이 부제(15)인데,
  코드는 그 반대였다(13 / 22). 아이콘도 이미지 에셋에서 명세 글리프로 바꿨다.
  퀘스트는 라임 원 66 + 깃발, 뱃지는 라운드 사각 66 + 어워드 — 원/사각이 둘을 구분한다.
  색종이·스태거 연출은 그대로 두었다.

  §20 쿠폰 사용은 손대지 않았다. 명세는 팝업이지만 앱은 이미 **화면**으로 구현돼 있어
  (바코드 확대·저장·공유까지) 팝업으로 되돌리는 것은 설계 후퇴로 보인다.
...
- **토스트 화면별 변주**: 그룹 채팅(잉크 + shield-check 21 + bottom 104)을 적용했다.
  그룹 정보 수정 토스트는 이미 자체 오버레이로 명세대로 구현돼 있었다.
  홈(라임·camera-check·bottom 112)·쿠폰함(라임·ticket)·목적지(current-location)는
  명세에 적힌 문안이 코드에 없어 적용할 자리가 없다. 해당 기능이 생길 때 함께 붙일 것.
